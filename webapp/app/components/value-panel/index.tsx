...
const TemplateVarPanel: FC<ITemplateVarPanelProps> = ({
  ...
  return (
    <div className={cn(isFold ? 'border border-border-subtle' : s.boxShodow, className, 'rounded-xl bg-surface')}>
      {/* header */}
      <div
        className={cn(isFold && 'rounded-b-xl', 'rounded-t-xl px-6 py-4 bg-accent-bg text-xs')}
      >
        {header}
      </div>
      {/* body */}
...
export const PanelTitle: FC<{ title: string, className?: string }> = ({
  ...
  return (
    <div className={cn(className, 'flex items-center space-x-1 text-content-accent')}>
      <StarIcon />
