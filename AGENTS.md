...
- **basePath**: `NEXT_PUBLIC_BASE_PATH` 环境变量驱动 `next.config.js` 的 `basePath`（页面路由 + 静态资源）和 `config/index.ts` 的 `API_PREFIX`（客户端 API 调用），默认为空（根路径）。所有客户端 `fetch('/api/...')` 统一使用 `API_PREFIX` 前缀
